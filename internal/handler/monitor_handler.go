...
func (h *MonitorHandler) List(c *gin.Context) {
	...
	type monitorWithTags struct {
		model.Monitor
		Tags            []model.Tag              `json:"tags"`
		Notifications   []uint                   `json:"notification_ids"`
	}
...
func (h *MonitorHandler) Update(c *gin.Context) {
	...
	if req.NotificationIDs != nil {
		h.DB.Where("monitor_id = ?", monitor.ID).Delete(&model.MonitorNotification{})
		for _, nid := range req.NotificationIDs {
			mn := model.MonitorNotification{MonitorID: monitor.ID, NotificationID: nid}
			h.DB.Create(&mn)
		}
...
func (h *MonitorHandler) Resume(c *gin.Context) {
	...
	if err := h.DB.Model(&model.Monitor{}).Where("id = ? AND user_id = ?", monitorID, userID).Update("active", true).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "monitor not found"})
		return
	}
...
func (h *MonitorHandler) Pause(c *gin.Context) {
	...
	if err := h.DB.Model(&model.Monitor{}).Where("id = ? AND user_id = ?", monitorID, userID).Update("active", false).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "monitor not found"})
		return
	}
