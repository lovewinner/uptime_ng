...
func TestRefreshMonitorAssociationsHonorsIndependentFlags(t *testing.T) {
	...
	oldNotif := model.Notification{UserID: 1, Name: "old", Type: "email", Config: `{"to":"old@example.com"}`, Active: true}
	newNotif := model.Notification{UserID: 1, Name: "new", Type: "email", Config: `{"to":"new@example.com"}`, Active: true}
